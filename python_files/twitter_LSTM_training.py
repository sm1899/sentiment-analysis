...
GRAPHS_DIR = "twitter_lstm_training_graphs"
LOGS_DIR = "twitter_lstm_training_logs"
MODEL_DIR = "twitter_lstm_models"
...
# Start logging
print(f"=== LSTM Sentiment Analysis Training Log ===")
...
class TwitterLSTMDataset(Dataset):
...
class LSTMSentimentClassifier(nn.Module):
    ...
    def forward(self, text):
        ...
        # We use the last hidden state for classification as specified
...
def plot_metrics(epochs, train_metrics, val_metrics, metric_name, fold):
    ...
    plt.title(f'LSTM Training and Validation {metric_name} - Fold {fold}')
    plt.xlabel('Epochs')
...
def plot_average_metrics(epochs, train_metrics_all_folds, val_metrics_all_folds, metric_name):
    ...
    plt.title(f'LSTM Average Training and Validation {metric_name} Across All Folds')
    plt.xlabel('Epochs')
    plt.ylabel(f'{metric_name}')
    plt.legend()
    plt.grid(True)
...
embedding_dim = 100
hidden_dim = 256  # As specified
output_dim = 3    # For multi-class classification (Negative, Neutral, Positive)
dropout = 0.3
learning_rate = 0.001
num_epochs = 10
batch_size = 64
...
for fold, (train_idx, val_idx) in enumerate(kf.split(token_indices)):
    ...
    model = LSTMSentimentClassifier(vocab_size, embedding_dim, hidden_dim, output_dim, dropout).to(device)
...
print("\nFile Outputs:")
print(f"- LSTM Training logs saved to: {log_filename}")
print(f"- LSTM Training graphs saved to: {GRAPHS_DIR}/ directory")
print(f"- LSTM Best models saved as: {MODEL_DIR}/lstm_best_model_fold[1-5].pt")
...
print("For multi-class classification (3 or more classes):")
print("- Used CrossEntropyLoss, which combines LogSoftmax and NLLLoss")
print("- Appropriate for mutually exclusive classes like Negative/Neutral/Positive")
print("\nFor binary classification (if needed):")
print("- Would use BCEWithLogitsLoss (Binary Cross Entropy with Logits)")
print("- Combines Sigmoid activation and BCELoss in one layer")
